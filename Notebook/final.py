...
# function for fetching tables
def fetch_all_data(table):
    return table

# function for fetching specific columns
...
# function for removing specific data from table
...
# function for modification of specific data
...
# function for the AGGREGATE command
...
# function for removing the entire table from the memory
...
# function to join tables and chunk them (chunk size can be increased or decreased according to need)
def join_tables(table1_name, table2_name, join_column, chunk_size=10000):
    ...
    try:
        ...
        # Split the result table into chunks and save each chunk
        for i in range(num_chunks):
            start_idx = i * chunk_size
            end_idx = min((i + 1) * chunk_size, num_rows)
            chunk = result_table.iloc[start_idx:end_idx]
...
# function for grouping by along with aggregation of the tables
...
# function for displaying table names and their respective columns
def display_tables():
    table_info = []
...
# function to define a new table with no values, all values or certain values
...
# function to load the existing tables in the memory along with chunking
...
# this is for the user interface using tkinter
